package mod.azure.azurelibarmor.loading;
...
public final class FileLoader {
	/**
	 * Load up and deserialize an animation json file to its respective {@link mod.azure.azurelibarmor.core.animation.Animation} components
	...
	public static String getFileContents(ResourceLocation location, ResourceManager manager) {
		try (InputStream inputStream = manager.getResourceOrThrow(location).open()) {
			...
			AzureArmorLib.LOGGER.error("Couldn't load " + location, e);
