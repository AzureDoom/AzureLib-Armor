...
/**
 * {@link com.google.gson.Gson} {@link JsonDeserializer} for {@link mod.azure.azurelibarmor.loading.object.BakedAnimations}.<br>
	...
	public BakedAnimations deserialize(JsonElement json, Type type, JsonDeserializationContext context) throws JsonParseException {
			...
			for(JsonElement entry : includeListJSONObj.asList()) {
				...
				for(JsonElement animName : obj.getAsJsonArray("animations")) {
					...
					if(includes.containsKey(ani)) {
						AzureArmorLib.LOGGER.warn("Animation {} is already included! File already including: {}  File trying to include from again: {}", ani, includes.get(ani).toString(), fileId.toString());
					} else {
		...
		for (Map.Entry<String, JsonElement> entry : animationJsonList.entrySet()) {
			try {
				...
				AzureArmorLib.LOGGER.error("Unable to parse animation: " + entry.getKey());
				ex.printStackTrace();
			}
		}
