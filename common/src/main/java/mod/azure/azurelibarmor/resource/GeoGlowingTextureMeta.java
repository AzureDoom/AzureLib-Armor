 ...
 */
package mod.azure.azurelibarmor.resource;

import java.util.List;

import mod.azure.azurelibarmor.renderer.layer.AutoGlowingGeoLayer;
...
/**
 * Metadata class that stores the data for AzureLib's {@link AutoGlowingGeoLayer emissive texture feature} for a given texture
 */
public class GeoGlowingTextureMeta {
	...
	public void createImageMask(NativeImage originalImage, NativeImage newImage) {
		for (Pixel pixel : this.pixels) {
			...
			if (pixel.alpha > 0)
				color = FastColor.ABGR32.color(pixel.alpha, FastColor.ABGR32.red(color), FastColor.ABGR32.green(color), FastColor.ABGR32.blue(color));
