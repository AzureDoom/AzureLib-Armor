...
public class AnimationState<T extends GeoAnimatable> {
    ...
    /**
     * Checks whether the current {@link AnimationController}'s last animation was the one provided. This allows for
     * multi-stage animation shifting where the next animation to play may depend on the previous one
     *
    ...
    /**
     * Similar to {@link AnimationState#isCurrentAnimation}, but additionally checks the current stage of the animation
     * by name.<br>
     ...
     * in the {@link RawAnimation} last provided
     *
     ...
     * This should be used in controllers when stopping a non-looping animation, so that it is reset to the start for
